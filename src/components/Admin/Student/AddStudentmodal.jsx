...
const AddStudentModal = ({ isOpen, onClose }) => {
  ...
  const [avatarPreview, setAvatarPreview] = useState(null);   // ⬅️ preview URL
  const [avatarFile, setAvatarFile]   = useState(null);       // ⬅️ File object
  ...
  const handleImageChange = (file) => {
    if (!file) return;
    setAvatarFile(file);
    setAvatarPreview(URL.createObjectURL(file)); // blob preview
  };
  ...
  const handleSubmit = (e) => {
    ...
    // For now, just log:
    console.table({ ...form, avatarFile });
    // TODO: send `avatarFile` + form data to API
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center">
      <div className="bg-white rounded-lg w-full max-w-2xl shadow-lg animate-fadeIn overflow-hidden">
        ...
        <form
          ...
          className="p-4 max-h-[75vh] overflow-y-auto space-y-4"
        >
          {/* Avatar upload */}
          <div
