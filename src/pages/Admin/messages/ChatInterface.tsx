...
const ChatInterface: React.FC<Props> = ({
  ...
  const messages: any = useSelector((state: RootState) => state.getMessage.listRecords);

  // detect if current chat is with teacher or admin
  const isTeacher = !!selectedTeacher;
  const isGuardian = !!selectedGuardian;

  const displayName = isTeacher
    ? `${selectedTeacher?.firstname ?? ""} ${selectedTeacher?.lastname ?? ""}`
    : `${selectedGuardian?.firstname ?? ""} ${selectedGuardian?.lastname ?? ""}`;

  const contactId = isTeacher ? selectedTeacher?.teacherId : selectedGuardian?.guardianId;
  ...
  useEffect(() => {
    if (user && (isTeacher || isGuardian)) {
      fetchMessages();
    }
  }, [dispatch, user, selectedTeacher, selectedGuardian, trigger]);

  const fetchMessages = async () => {
    dispatch(fetchMessageStart());
    try {
      if (contactId && contactRole) {
        const res = await messageService.getMessageByUserId(contactId, contactRole);
      ...
      <div className="px-4 py-2 border-b border-gray-200 bg-orange-100">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-3">
            ...
            />
            <div>
              <h3 className="font-medium text-gray-900 text-sm lg:text-base">{displayName}</h3>
      ...
      <div className="flex-1 overflow-y-auto parent-scrollbar p-3 lg:p-4 space-y-3 lg:space-y-4">
        {sortedMessage.length === 0 ? (
          ...
          sortedMessage.map((m) => (
            <div
              key={m.messageId}
              className={`flex ${m.senderRole ? "justify-end" : "justify-start"}`}
            >
              <div
                className={`max-w-[75%] lg:max-w-xs px-3 lg:px-4 py-2 rounded-lg ${
                  m.senderRole ? "bg-orange-500 text-white" : "bg-gray-200 text-gray-900"
                }`}
              >
                {m.type === "voice" ? (
                ...
                <div
                  className={`text-[10px] lg:text-xs mt-1 ${
                    m.senderRole ? "text-orange-100" : "text-gray-500"
                  }`}
                >
                  {formatTime(m.sentAt)}
