...
export const initialExaminationForm: ExaminationForm = {
  title: "", examType: "INTERNAL", term: "FIRST",
  academicSession: "", durationMinutes: 60, classLevel: "",
  subjectId: "", scheduledDate: "", passingScore: "", instructions: "",
};
...
export default function AdminCbtExaminationsPage() {
  ...
  const fetchExaminations = async (page = 1) => {
    ...
    try {
      ...
      setPagination({ pageNumber: res.pageNumber, pageSize: res.pageSize, totalCount: res.totalCount, totalPages: res.totalPages });
    } catch (err) {
      console.error(err);
    } finally {
      setFetching(false);
  ...
  useEffect(() => { fetchExaminations(1); }, [cbtUser?.schoolId]);
      ...
      setExaminations((prev) =>
        prev.map((e) =>
          e.id === assignTarget.id
            ? { ...e, examinerId: selectedExaminers[0] ?? null }
            : e
        )
      );
  ...
  const filtered = useMemo(() => {
    ...
    return examinations.filter((e) =>
      `${e.title} ${e.examType} ${e.term} ${e.status} ${e.academicSession}`.toLowerCase().includes(q)
    );
  }, [examinations, search]);

  /* ── Create / Edit ── */
  const openAdd = () => { setForm(initialExaminationForm); setEditing(null); setModal(true); };

  const openEdit = (exam: Examination) => {
    setForm({
      title: exam.title, examType: exam.examType, term: exam.term,
      academicSession: exam.academicSession, durationMinutes: exam.durationMinutes,
      classLevel: exam.classLevel ?? "", subjectId: exam.subjectId,
  ...
  const save = async () => {
    ...
    try {
      const payload = {
        title: form.title, examType: form.examType, term: form.term,
        academicSession: form.academicSession, duration: String(form.durationMinutes),
        classLabel: form.classLevel, subjectId: form.subjectId,
        scheduledDate: form.scheduledDate || undefined,
        passingScore: form.passingScore ? Number(form.passingScore) : 0,
        instructions: form.instructions, schoolId: cbtUser?.schoolId ?? "",
      };
      if (editing) {
        await AdminCbtExaminationService.update(editing, payload);
    ...
    <div className="min-h-screen font-sans">
      <div className="mx-auto max-w-6xl space-y-6 p-6">
        ...
        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-orange-500">Administration</p>
          ...
          <Btn onClick={openAdd} disabled={fetching}><Icons.Plus />Create Exam</Btn>
        </div>

        {/* Stats */}
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            { label: "Total Exams",  value: pagination.totalCount,                                        icon: "ti-book",           accent: "bg-orange-500", ring: "ring-orange-100", text: "text-orange-600", bg: "bg-orange-50"  },
            { label: "This Page",    value: examinations.length,                                           icon: "ti-layout-list",    accent: "bg-slate-400",  ring: "ring-slate-100",  text: "text-slate-500",  bg: "bg-slate-50"   },
            { label: "Total Pages",  value: pagination.totalPages,                                         icon: "ti-file-stack",     accent: "bg-blue-500",   ring: "ring-blue-100",   text: "text-blue-600",   bg: "bg-blue-50"    },
            { label: "Scheduled",    value: examinations.filter((e) => e.status === "SCHEDULED").length,  icon: "ti-calendar-check", accent: "bg-emerald-500",ring: "ring-emerald-100",text: "text-emerald-600",bg: "bg-emerald-50" },
          ].map(({ label, value, icon, accent, ring, text, bg }) => (
            <div key={label} className="relative overflow-hidden rounded-2xl border border-gray-100 px-5 py-4 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md">
              <div className={`absolute inset-x-0 top-0 h-1 rounded-t-2xl ${accent}`} />
              <div className="mt-1 flex items-start justify-between">
                <div>
                  <p className="text-[10px] md:text-xs font-semibold uppercase tracking-widest text-gray-400">{label}</p>
                  <p className="mt-1.5 text-2xl md:text-3xl font-black tracking-tight text-gray-900">{value}</p>
                </div>
                <div className={`flex h-8 w-8 md:h-10 md:w-10 items-center justify-center rounded-xl ${bg} ring-4 ${ring}`}>
        ...
        <div className="overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-sm">
          <div className="flex items-center gap-3 border-b border-gray-100 px-5 py-4">
            <div className="relative flex-1">
              <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-300"><Icons.Search /></span>
              <input
                value={search}
            ...
            <Badge color="orange" className="whitespace-nowrap">{filtered.length} shown</Badge>
          </div>
          ...
          {pagination.totalPages > 1 && (
            <div className="flex items-center justify-between border-t border-gray-100 px-5 py-3">
              <p className="text-xs text-gray-400">
                Page <span className="font-semibold text-gray-600">{pagination.pageNumber}</span> of {pagination.totalPages} · {pagination.totalCount} records
              </p>
              <div className="flex gap-2">
                <Btn size="sm" variant="outline" disabled={pagination.pageNumber <= 1 || fetching} onClick={() => fetchExaminations(pagination.pageNumber - 1)}>Previous</Btn>
                <Btn size="sm" variant="outline" disabled={pagination.pageNumber >= pagination.totalPages || fetching} onClick={() => fetchExaminations(pagination.pageNumber + 1)}>Next</Btn>
              </div>
            </div>
          )}
        </div>
      ...
      {/* Create / Edit modal */}
      <Modal open={modal} onClose={() => !submitting && setModal(false)} title={editing ? "Edit Examination" : "Create Examination"}>
        <AdminCbtExaminationForm form={form} setForm={setForm} editing={editing} onCancel={() => setModal(false)} onSubmit={save} submitting={submitting} />
      </Modal>

      {/* Assign Examiner modal */}
      <Modal open={assignModal} onClose={() => !assigning && setAssignModal(false)} title={`Assign Examiners — ${assignTarget?.title ?? ""}`}>
        <div className="space-y-4">
          {fetchingExaminers ? (
            ...
            <p className="py-8 text-center text-sm text-gray-400">No examiners found for this school.</p>
          ) : (
            <div className="max-h-72 space-y-2 overflow-y-auto pr-1">
              {examiners.map((examiner) => {
                const selected = selectedExaminers.includes(examiner.id);
                const initials = examiner.fullname.split(" ").map((n) => n[0]).slice(0, 2).join("");
                const hue = examiner.fullname.split("").reduce((a, c) => a + c.charCodeAt(0), 0) % 360;
                return (
                  <button
                    key={examiner.id}
          ...
          <div className="flex gap-3">
            <Btn variant="outline" onClick={() => setAssignModal(false)} disabled={assigning} className="flex-1">Cancel</Btn>
            <Btn onClick={saveAssignment} disabled={assigning || selectedExaminers.length === 0} loading={assigning} className="flex-1">
      ...
      {/* Delete dialog */}
      <ConfirmDialog open={!!deleteTarget} onClose={() => !deleting && setDeleteTarget(null)} onConfirm={remove} loading={deleting} />
    </div>
  );
}