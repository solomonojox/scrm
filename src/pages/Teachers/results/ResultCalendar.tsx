...
const ResultCalendar = (): React.JSX.Element => {
  const [currentDate, setCurrentDate] = useState(new Date());
  const navigate =  useNavigate();

  const toggleNewResult = () => {
    navigate("/teacher/new-result");
  };
          ...
          <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between px-4 sm:px-6 lg:px-8 py-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-2 gap-4">
              <div className="flex items-center space-x-4">
                <h1 className="text-xl sm:text-2xl font-semibold text-gray-900">Result</h1>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              <button onClick={toggleReportCard} className="inline-flex items-center justify-center text-xs sm:text-sm px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white font-medium rounded-md transition-colors shadow-sm w-full sm:w-auto">
                <FileText className="w-4 h-4 mr-2" />
                View Report Card
              </button>

              <button
                onClick={toggleNewResult}
